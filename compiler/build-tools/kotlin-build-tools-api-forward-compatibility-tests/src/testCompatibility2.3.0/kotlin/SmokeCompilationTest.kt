...
class SmokeCompilationTest : BaseCompilationTest() {
    ...
    fun testIncrementalCompilationWithClasspathSnapshot(strategyConfig: CompilerExecutionStrategyConfiguration) {
        ...
        toolchain.createBuildSession().use { session ->
            fun snapshotLib() {
                session.executeOperation(toolchain.jvm.createClasspathSnapshottingOperation(libDestination))
    ...
    private fun jvmIncrementalCompilationOperation(
        ...
        this[JvmCompilationOperation.INCREMENTAL_COMPILATION] = JvmSnapshotBasedIncrementalCompilationConfiguration(
            ...
            this.options[JvmSnapshotBasedIncrementalCompilationOptions.ROOT_PROJECT_DIR] = workingDirectory
        }
    }
    ...
    private fun jvmNonIncrementalCompilationOperation(
        ...
        destination: Path,
        extraClasspath: List<Path> = emptyList(),
        additionalConfiguration: JvmCompilationOperation.() -> Unit = {},
    ): JvmCompilationOperation = toolchain.jvm.createJvmCompilationOperation(sources, destination).apply {
